...
class SensorBase(ABC):
    ...
    def __init__(self, config: Dict):
        ...
        self.prim_path = self.config["prim_path"]
    ...
    def reset(self, env_ids: torch.Tensor[int] | None = None):
    ...
    def update(self, dt: float, force_recompute: bool = False):
        ...
        self._timestamp += dt
        self._is_outdated |= self._timestamp - self._timestamp_last_update + 1e-6 >= self.update_period
        ...
        if force_recompute or (self.history_length > 0):
            self._update_outdated_buffers()
    ...
    def _initialize_impl(self):
        ...
        env_prim_path_expr = self.prim_path.rsplit("/", 1)[0]
        ...
        # Current timestamp (in seconds)        # check that config is valid
        if self.history_length < 0:
            raise ValueError(f"History length must be greater than 0! Received: {self.history_length}")
    ...
    @abstractmethod
    def _update_buffers_impl(self, env_ids: torch.Tensor[int]):
